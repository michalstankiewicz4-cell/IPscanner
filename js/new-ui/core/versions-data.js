  ...
  var versions = [
    {
      ...
      notes: [
        ...
        "The bottom Terminal is now a real interactive console instead of a run-and-wait one: output streams live as it's produced (a long-running command like \"ping -t\" shows each line as it arrives) and Ctrl+C actually interrupts whatever's currently running - killing the whole process tree, not just the top-level shell - instead of the app just sitting there until it finishes on its own. Clicking the command row or the Terminal tab while something is running also counts as focusing the terminal, so Ctrl+C works without an extra click into the output area first.",
        "Added floating quick-command buttons over the Terminal's output pane, shown when the IP Scanner tool is open (netstat / ipconfig / ping): left click runs the bare command immediately, right click just types it into the input so you can add your own arguments before pressing Enter.",
        "The Terminal's input now supports Up/Down arrow history navigation like a real shell, cycling through previously run commands and back to whatever you were mid-typing. History is saved locally and also bundled into the session file (a new, separate table), so it survives both an app restart and loading an older session.",
        "Removed the leftover local `tools/` addon folder and its old browsing mechanism - fully replaced by the GitHub-topic-based Community Catalog, so it was no longer read by anything.",
        "Mail XSS Tester: added Onet (smtp.poczta.onet.pl) as a second SMTP provider alongside Gmail, picked from a dropdown - lets a test message actually cross two independent SMTP hops instead of always going straight from this app's own connection.",
        "Mail XSS Tester: added a Custom Technique Builder (vector: <script>/<style> x mechanism: soft line-break, hex-escape the opening/closing/both angle brackets, or a real natural quoted-printable line wrap with an editable filler-text field) - build and send a specific combination on demand instead of needing a new hardcoded checkbox for every variant, plus a \"+\" queue to batch several combinations and send them together.",
        "Mail XSS Tester: removed 16 fixed technique checkboxes that the Custom Technique Builder can fully reproduce, keeping only the 7 raw-MIME techniques it can't build (MIME boundary desync, UTF-7 charset, overlong UTF-8, encoded-word header, etc.).",
        "Mail XSS Tester: sends made through the Custom Technique Builder (single or queued) now also appear in the results table's Triggered/Not-triggered list, not just the fixed checkboxes.",
        "Fixed a real bug where mail ownership verification always sent its one-time code through Gmail's SMTP even when a different provider was selected for the sender - verification now has its own fully independent provider/address/password fields instead of sharing them with Mail XSS Tester.",
        "Fixed mail ownership verification requiring the public beacon tunnel to be running before it would send - verification emails carry no beacon URL at all, so that requirement was an unnecessary leftover.",
        "README and the in-app About tab now explicitly disclose that this is also an active security-testing toolkit (Mail XSS Tester, Browser Inspect, WiFi password reveal) alongside the IP/port scanner, with an authorized-use-only disclaimer.",
        "Added passive TCP banner grabbing (Options -> Config -> Detect -> \"Banner Grabbing\"): after a TCP connect succeeds, captures whatever a service sends unprompted (SSH/FTP/SMTP/POP3/IMAP-style greetings, never sending anything itself) into the Results table's existing \"Banner Grabbing\" column, which previously always showed \"-\". Opt-in, off by default.",
        "Fixed Port Presets' name/ports/emoji fields losing keyboard focus after every single keystroke, forcing a re-click before typing the next character."
      ]
    },
    {
      version: "v2.8.6",
      notes: [
        ...
        "Added a third scan mode, Memory (next to Range/CIDR in the IP Scanner sidebar): type or paste a hand-picked list of IP addresses into a notepad-style tab (reachable from the sidebar radio, the tab strip, or Options -> Memory) and scan exactly that list instead of sweeping a range. Every address shows up in the results table as pending the moment you hit Start, and any that never respond flip to \"no response\" once the scan finishes.",
        "Added a \"Copy to Memory\" button to the IP Extractor (next to Add/Extract, greyed out when the extracted list is empty): copies every extracted address into the Memory notepad, skipping ones already there, and switches straight to the Memory tab so the result is visible right away instead of only showing up in the status line.",
        "The IP Extractor's input box and its extracted-address list now survive a reload/restart (previously reset to empty every time) and, together with the Memory notepad's content, are now included in saved session files, the same way scan results and port presets already are.",
        "CIDR mode: typing a bare address with no \"/n\" (e.g. \"10.1.2.3\") now scans just that one host instead of being rejected - treated the same as an explicit \"/32\".",
        "Memory notepad's list can now also contain IPv6 addresses - bare, or with a Windows zone id like \"fe80::1%9\" for link-local addresses - freely mixed with IPv4 entries and CIDR blocks on the same list. CIDR blocks are still IPv4-only and get expanded into every address in the range; IPv6 entries are always scanned exactly as typed, never as a range (an IPv6 subnet is far too large to brute-force the way IPv4 CIDR does).",
        "Added a confirm dialog (OK/Cancel) before starting any scan - Range, CIDR, or Memory - that covers more than 256 addresses, so a mistyped wide CIDR prefix or a large pasted list doesn't silently kick off a huge scan with no chance to back out."
      ]
    },
    {
      version: "v2.8.5",
